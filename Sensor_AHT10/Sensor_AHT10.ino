/*n
 * Programa que efetua a leitura de um sensor de temperatura e umidade AHT10 e envia para o servidor MQTT Broker.
 ...
 * Branch: AHT10
 * Ajeitar SetUpAP para ter verificações.
 * */
...
void sendMQTTJson_TempHumidity(const float, const float);            // Envia o payload com temperaturas em Celcius e Fahrenheit para o Broker
charArray createJson_TempHumidity(const float, const float);      // Cria o payload com temperaturas em Celcius e Fahrenheit
...
void setup() {
  ...
  while (!aht.begin()) {
    Serial.println("Não foi possível encontrar o sensor AHT10. Verifique a conexão!");
  }
...
void loop() {
  ...
  if (currentMillis - runTimeData.previousMillis >= mqttConfig.intervalDelay) {
    ...
    aht.getEvent(&runTimeData.humidityEvent, &runTimeData.tempEvent);

    if (!isnan(runTimeData.tempEvent.temperature) && !isnan(runTimeData.humidityEvent.relative_humidity)) {
      sendMQTTJson_TempHumidity(runTimeData.tempEvent.temperature, runTimeData.humidityEvent.relative_humidity);
    }
  }
}
...
bool isThereConfigFile() {
  if (LittleFS.begin(true)){
    ...
    Serial.println("isThereConfigFile: Falha ao iniciar SPIFSS");
    return false;
  }
}
...
void setUpAP() {
  ...
  WiFiManagerParameter Port("Porta", "Porta do Broker", "1883", sizeof(mqttConfig.brokerPort));
...
charArray createJson_TempHumidity(const float temp, const float humidity) {
  charArray payload(new char[appSettings.PAYLOAD_BUFFER_SIZE]);
  snprintf(payload.get(), appSettings.PAYLOAD_BUFFER_SIZE, "{\"temperatura\": %.2f , \"umidade\": %.2f}", temp, humidity);
  return payload;
}
...
void sendMQTTJson_TempHumidity(const float temp, const float humidity) {
  charArray payloadTempHum = createJson_TempHumidity(temp, humidity);
  Serial.println(payloadTempHum.get());
  mqttClient.publish(mqttConfig.topic, payloadTempHum.get());
}
