...
void loop() {
  ...
  if (currentMillis - runTimeData.previousMillis >= mqttConfig.intervalDelay) {
    ...
    ds18b20.requestTemperatures(); // Solicita a leitura da temperatura
    runTimeData.temperatureCelsius = ds18b20.getTempCByIndex(0); // Armazena a temperatura

    // O DS18B20 retorna -127 em caso de falha na leitura.
    if (!isnan(runTimeData.temperatureCelsius) && runTimeData.temperatureCelsius != DEVICE_DISCONNECTED_C) {
      sendMQTTJson_Temp(runTimeData.temperatureCelsius);
    } else {
      Serial.println("Erro ao ler dados do sensor DS18B20.");
    }
  }
}
